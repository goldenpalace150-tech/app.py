...
def load_attendance_data_from_api(selected_date_str, selected_date_obj):
    ...
    for code, name in active_employees.items():
        # Sort and run absolute duplicate punch removal logic
        raw_user_punches = sorted(emp_punches.get(code, []), key=lambda item: item[0])
        
        # 🛡️ THE BULLETPROOF DEDUPLICATOR: Erases exact double-punches generated by the server inside the same minute
        user_all_punches = []
        for current in raw_user_punches:
            if not user_all_punches:
                ...
                # If time difference between consecutive logs is less than 61 seconds, treat it as a duplicate ghost entry and skip it
