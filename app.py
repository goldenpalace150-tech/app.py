...
def parse_biotime_basic_report(report_bytes):
  ...
  Returns:
      dict[BioTime ID][date] = {
          "Clock In", "Clock Out", "Actual WT", "Total WT"
      }
  ...
  Actual WT/Total WT table. Actual WT is intentionally preferred because it
  ...
  for ws in wb.worksheets:
    for row in ws.iter_rows(values_only=True):
      ...
      if normalized_header[:5] == ["date", "clock in", "clock out", "actual wt", "total wt"]:
        reading_rows = True
        continue
      ...
      try:
        ...
        for index, attendance_date in enumerate(dates_to_fetch, start=1):
          ...
          for employee_id, report_row in report_date_map.items():
            ...
            actual_wt = report_row.get("Actual WT") or report_row.get("Total WT")
            if actual_wt:
